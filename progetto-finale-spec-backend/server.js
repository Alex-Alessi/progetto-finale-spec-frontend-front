...
const loadPromises = resourceTypes.map((type) => {
  ...
  // 📌 PATCH /:resource/:id - Update partial fields
  app.patch(`/${pluralType}/:id`, async (req, res) => {
    const itemId = parseInt(req.params.id);
    const itemIndex = cache[type].findIndex((p) => p.id === itemId);
    if (itemIndex === -1) {
      return res.status(404).json({
    ...
    const typeReadonlyProps = readonlyProperties[type] || [];
    const readonlyAttempts = Object.keys(updatedFields).filter((key) =>
      typeReadonlyProps.includes(key)
    );
    ...
    if (readonlyAttempts.length > 0) {
      return res.status(400).json({
        ...
        error: `Cannot update readonly properties`,
        details: {
          readonly: readonlyAttempts,
          message: `Readonly fields: ${readonlyAttempts.join(", ")}`,
        },
      });
    }

    // Validate with merged data
    const newItem = { ...existingItem, ...updatedFields };
    const validationResult = validator(newItem);
    if (!validationResult.valid) {
      return res.status(400).json({
        error: `Invalid ${type} data`,
        details: validationResult.errors,
